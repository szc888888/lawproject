...
public class OpenAIWebSocketEventSourceListener extends EventSourceListener {
    ...
    public OpenAIWebSocketEventSourceListener(Session session,Long userId,AsyncService asyncService,String msg,MsgMoneyVo msgMoneyVo) {
        this.session = session;
        this.userId = userId;
        this.msg = msg;
        this.asyncService = asyncService;
        this.msgMoneyVo = msgMoneyVo;
        this.conversationId="";
    }
    ...
    public void onEvent(EventSource eventSource, String id, String type, String data) {
        ...
        if (data.equals("[DONE]")) {
            ...
            //这一步需要扣除用户次数
            asyncService.updateUserMsgCountAndLogByQuestionsAndAnswersOnly(userId,msg,msgMoneyVo,0);
            //存提问记录和回答
            log.info("问题是:"+msg);
            log.info("回答是:"+lastMessage);
            asyncService.saveQuestionAndAnswer(conversationId,userId,msg,lastMessage,msgMoneyVo);
            return;
        }
...
//        }
    ...
    public void onFailure(EventSource eventSource, Throwable t, Response response) {
        ...
        session.getBasicRemote().sendText("GPT可能出现了些许小问题,正在赶修!");
        session.getBasicRemote().sendText("[DONE]");
        eventSource.cancel();
    }
}
