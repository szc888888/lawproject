...
import io.renren.common.gpt.OpenAiClientNoSdk;
...
//        BalanceDTO balanceDTO = getBalanceDTO(dto.getApiKey(),dto.getType());
...
//        GptKeyDTO data = gptKeyService.get(id);
//        BalanceDTO balanceDTO = getBalanceDTO(data.getApiKey(), data.getType());
//        return new Result<BalanceDTO>().ok(balanceDTO);
        return new Result<BalanceDTO>().error("因官方接口变更,查询余额暂不可用");
    }
    public BalanceDTO getBalanceDTO(String key,Integer type){
        try {
            OpenAiStreamClient chatGPTStream = getChatGPTStream(key,type);
            double hardLimitUsd1 = 0;
            String accName = "";
            if(type == 2){
                Subscription subscription = chatGPTStream.subscription();
                hardLimitUsd1 = subscription.getHardLimitUsd();;
                accName = subscription.getAccountName();
            }else{
                SubscriptionSon subscription = getSubscription(key);
                hardLimitUsd1 = subscription.getHardLimitUsd();;
                accName = subscription.getAccountName();
            }
            ...
            //剩余多少
            BigDecimal hardLimitUsd = new BigDecimal(hardLimitUsd1).setScale(2, BigDecimal.ROUND_HALF_UP);//美元保留2位小数并且四舍五入
            ...
            balanceDTO.setAccountName(accName);
            return balanceDTO;
        }catch (Exception e){
            throw new RenException("KEY校验错误或者链接超时");
        }
    }
    public OpenAiStreamClient getChatGPTStream(String key,Integer type){
        if(type == null){
            type = 0;
        }
        String chatGptUrl = null;
        if(type == 2){//三方的key
            chatGptUrl = sysParamsService.getValue("SANFANG_CHAT_GPT_URL");
            if(StrUtil.isBlank(chatGptUrl)){
                throw new RenException("请先配置三方GPT代理，再添加三方GPTKEY");
            }
        }else {
...
//                .proxy(proxy)
    ...
    private SubscriptionSon getSubscription(String key) {
       String chatGptUrl = sysParamsService.getValue("CHAT_GPT_URL");
        if(StrUtil.isBlank(chatGptUrl)){
            chatGptUrl = "https://api.openai.com/";
        }
        OpenAiClientNoSdk chatGPTStream;
        HttpLoggingInterceptor httpLoggingInterceptor = new HttpLoggingInterceptor(new OpenAILogger());
        //!!!!!!测试或者发布到服务器千万不要配置Level == BODY!!!!
        //!!!!!!测试或者发布到服务器千万不要配置Level == BODY!!!!
        httpLoggingInterceptor.setLevel(HttpLoggingInterceptor.Level.HEADERS);
...
//                .proxy(proxy)
                ...
                .readTimeout(600, TimeUnit.SECONDS)
                .build();
        chatGPTStream = OpenAiClientNoSdk
                .builder()
                .apiHost(chatGptUrl)
                .apiKey(Arrays.asList(key))
                //自定义key使用策略 默认随机策略
                ...
                .okHttpClient(okHttpClient)
                .build();
        SubscriptionSon subscription = chatGPTStream.subscription();
        return subscription;
    }
}